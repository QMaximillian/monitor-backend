    ...
    Query: {
      ...
      upcoming_appointment: async (parent, {audition_id}, context) => {
              ...
              const upcoming_appointment =
                  await knex
                  .select("appointments.start_time", "appointments.id", "users.id AS user_id", "users.first_name", "users.last_name", "users.email", "users.phone_number")
                  .from("appointments")
                  .leftJoin('users', 'users.id', 'appointments.user_id')
                  .where('audition_id', audition_id)
                  .andWhere(knex.raw(`appointments.start_time::timestamp`), ">", knex.raw(`current_timestamp AT TIME ZONE 'utc'`))
                  ...
                  return { id, start_time , user: { id: user_id, first_name, last_name, email, phone_number }}
                } else {
                  return null
                }
              } catch(error) {
